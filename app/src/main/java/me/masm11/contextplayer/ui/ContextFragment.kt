...
class ContextFragment: Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val view = inflater.inflate(R.layout.context_fragment, container, false)
	...
	items.sortWith(object: Comparator<Item> {		// lambda だけで書けない…?
        ...
        view.findViewById<RecyclerView>(R.id.context_list).apply {
	...
	itemDecor.attachToRecyclerView(view.findViewById<RecyclerView>(R.id.context_list))

	itemAdapter.setOnClickListener { item ->
	    playContexts.setCurrent(item.ctxt)
	    val c = context
	    if (c != null)
		PlayerService.switchContext(c)
	}
